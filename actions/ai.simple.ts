...
export const getAiResponse = async (
  prompt: string,
  modelId: OllamaChatModelId = 'taozhiyuai/llama-3-8b-ultra-instruct:q2_k', // or 'phi3:mini'
  modelSettings: OllamaChatSettings | undefined = undefined
) => await generateText({
  prompt,
  model: ollama(modelId, modelSettings),
  system: 'You are a helpful assistant who gives accurate and brief answers.',
  temperature: 0.8,
  frequencyPenalty: 0.6,
  seed: Date.now() + randomInt(0, 416)
})
